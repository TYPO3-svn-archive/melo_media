<?php
	...
	/** 
	 * Plugin 'FLV Player' for the 'videoplayer' extension.
	 *
	...
	class tx_videoplayer_pi1 extends tslib_pibase {
		...
		// Same as class name
		var $prefixId = 'tx_videoplayer_pi1';
		
		// Path to this script relative to the extension dir
		var $scriptRelPath = 'pi1/class.tx_videoplayer_pi1.php';
		
		// The extension key
		var $extKey = 'video_player';
		
		// Upload directory
		var $uploadDir = 'uploads/tx_videoplayer/';
		 ...
		 * 
		 * This function initialises the plugin "tx_videoplayer_pi1", and
		...
		function buildFlashCode() {
			...
			$swfPath = str_replace(PATH_site,'',t3lib_div::getFileAbsFileName($this->conf['videoplayer']));
			...
			if($this->conf['url']){
				...
				// File path
				$filePath = t3lib_div::getIndpEnv('TYPO3_SITE_URL').str_replace(PATH_site,'',t3lib_div::getFileAbsFileName($this->conf['file']));
				$filePath = str_replace(t3lib_div::getIndpEnv('TYPO3_REQUEST_HOST'),'',$filePath);
			}
						
			$extPath = t3lib_div::getIndpEnv('TYPO3_SITE_URL').str_replace(PATH_site,'',t3lib_extMgm::extPath('videoplayer'));
			...
			$GLOBALS['TSFE']->additionalHeaderData [$this->pi1->prefixId] = '<script type="text/JavaScript" src="'.t3lib_extMgm::siteRelPath("videoplayer").'pi1/AC_OETags.js"></script>';
					...
					if (hasRightVersion) {  // if we\'ve detected an acceptable version
						AC_FL_RunContent(
							...
							"flashvars","width='.$this->conf['width']
								...
								.'&file='.$filePath
								.'&autostart='.$autoStart
								.'&image=uploads/tx_videoplayer/'.$this->conf['image']
	 ...
	 * XCLASS inclusion
	 */
	if (defined('TYPO3_MODE') && $TYPO3_CONF_VARS[TYPO3_MODE]['XCLASS']['ext/video_player/pi1/class.tx_videoplayer_pi1.php']) {
		include_once($TYPO3_CONF_VARS[TYPO3_MODE]['XCLASS']['ext/video_player/pi1/class.tx_videoplayer_pi1.php']);
	}
?>
